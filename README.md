...
## 必要なもの
コマンドプロンプトまたはPowerShellを利用して、以下のコマンドを実行します。

> [!Tip]
> `Win + R` を押下して開くポップアップに `wt` または `cmd` または `powershell` と入力するとシェルが起動します。

> [!Note]
> Windows用の .bat ファイルは、以下の必要なソフトウェアも自動でインストールしますが、その場合スクリプトの再実行やPCの再起動が必要になるかもしれません。
> 予め手動でインストールしておくことをオススメします。
...
> [!Tip]
> 起動した黒い画面が止まり、 `サービスの起動に成功しました` が表示されたら起動完了です。  
> サービス起動後は、その画面を閉じてもPCを再起動するまではずっとRevolutionsを使える状態です。
